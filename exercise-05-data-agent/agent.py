...
def validate_sql(sql: str) -> None:
    ...
    # GOAL: Raise a ValueError if the normalized sql contains illegal expressions.
    # YOUR TASK: Whenever any of the following conditions is given, raise a
    # ValueError with:
    #   if (condition):
    #       raise ValueError("root cause explanation")
    #
    # Conditions to check for:
    # i) the normalized_sql statement starts with any other string than "select"
    #   or "SELECT"
    # ii) the normalized_sql statement contains a semicolon (";") (this may be
    #   an attempt to execute multiple sql statements at once)
    # iii) the normalized_sql statement contains "--" or "/*" or "*/" (without
    #   the double quotes) comments
    # iv) the normalized_sql contains any of the forbidden keywords
    # --->
    # YOUR CODE HERE:
    condition_1 = True
    if condition_1:
        raise ValueError("explanation 1")
    
    condition_2 = True
    if condition_2:
        raise ValueError("explanation 2")
    # ...
    # <---
...
def run_agent_loop(client: OpenAI, conn: sqlite3.Connection, schema: str, conversation_history):
    while True:
        ...
        try:
            # GOAL: Generate the SQL statement for the given query.
            # YOUR TASK: Call generate_sql_with_llm using four distinct arguments:
            # i) the OpenAI client
            # ii) the user's inquiry
            # iii) the DB schema string
            # iv) the conversation history
            # Store the result in a variable sql_raw.
            # Then send the raw sql to the function extract_sql, which should sanitize
            # the sql statement and remove unnecessary markdown. Store the result in
            # a variable sql. 
            # -->
            sql_raw = None # YOUR CODE HERE
            sql = None # YOUR CODE HERE
            # <---
